...
Paso 2 de la secuencia maestra (onboarding) construido — **NO LISTA todavía** (ver "Problemas conocidos" → veredicto:onboarding). Paso 1 (landing) también sigue NO LISTA, pausado por decisión del usuario para avanzar. Ambas pantallas compilan, tipan limpio y funcionan de punta a punta en el navegador.

Onboarding: flujo de 10 pasos en `/onboarding` (2 preguntas eco de dolores → reconocimiento → objeción dominante → meta → horas dormidas [slider] → hora de desconexión [slider, commitment device] → reconocimiento con etiquetado positivo → loading animado con labor illusion → resultado real calculado con los datos del usuario). CTA final lleva a `/paywall` (placeholder honesto — Paso 3 aún no construido).
